...
#define IOUT_AUTO_CALIBRATION 	0xe0
#define PMB_PMBUS_PWD	    	0xe1
#define TELEMETRY_AVERAGE 		0xe2
#define ADDRESS_CONFIG			0xe4
#define OTP_HYSTERESIS			0xe5
#define GPIO_CONFIG				0xe6
#define CLREF_DUTY_CYCLE		0xe7
#define PMB_BLACK_BOX_PWD   	0xf0
#define BLACK_BOX_VIN			0xf1
#define BLACK_BOX_VOUT_AVG		0xf2
#define BLACK_BOX_VOUT_MAX		0xf4
#define BLACK_BOX_FAULTS		0xf6
#define BLACK_BOX_STATUS		0xf7
#define BLACK_BOX_CONFIG		0xfc
#define PMB_NVM_PWD	        	0xfd
...
{ .cmd = PMB_USER_DATA(2),  .tag = "user_data_02", .type = RW2, .units = BITS, },
{ .cmd = PMB_USER_DATA(3),  .tag = "user_data_03", .type = RW2, .units = BITS, },
{ .cmd = PMB_USER_DATA(4),  .tag = "user_data_04", .type = RW2, .units = BITS, },
{ .cmd = PMB_USER_DATA(5),  .tag = "user_data_05", .type = RW2, .units = BITS, },
{ .cmd = PMB_USER_DATA(6),  .tag = "user_data_06", .type = RW2, .units = BITS, },
{ .cmd = PMB_USER_DATA(7),  .tag = "user_data_07", .type = RW2, .units = BITS, },
{ .cmd = PMB_USER_DATA(8),  .tag = "user_data_08", .type = RW2, .units = BITS, },
{ .cmd = PMB_USER_DATA(9),  .tag = "user_data_09", .type = RW2, .units = BITS, },
{ .cmd = PMB_USER_DATA(10), .tag = "user_data_10", .type = RW2, .units = BITS, },
{ .cmd = PMB_USER_DATA(11), .tag = "user_data_11", .type = RW2, .units = BITS, },

{ .cmd = IOUT_AUTO_CALIBRATION,   .tag = "IOUT_AUTO_CALIBRATION", .type = RW1, .units = BITS, },
{ .cmd = PMB_PMBUS_PWD	    	, .tag = "PMB_PMBUS_PWD", .type = RW2,  .units = BITS, },
{ .cmd = TELEMETRY_AVERAGE 		, .tag = "TELEMETRY_AVERAGE", .type = RW2,  .units = BITS, },
{ .cmd = ADDRESS_CONFIG			, .tag = "ADDRESS_CONFIG", .type = RW2,  .units = BITS, },
{ .cmd = OTP_HYSTERESIS			, .tag = "OTP_HYSTERESIS", .type = RW2,  .units = BITS, },
...
{ .cmd = PMB_NVM_PWD            , .tag = "PMB_NVM_PWD", .type = RW2, .units = BITS, },
...
//#endif
	...
	if(pmb_pwd){
		pmbus_write_word_reg(dev.fd, PMB_PMBUS_PWD, "PMB_PMBUS_PWD", 0x82c2);
	}else{
		pmbus_write_word_reg(dev.fd, PMB_PMBUS_PWD, "PMB_PMBUS_PWD", 0);
	}
	if(bb_pwd){
		pmbus_write_word_reg(dev.fd, PMB_BLACK_BOX_PWD, "PMB_BLACK_BOX_PWD", 0xBB40);
	}else{
		pmbus_write_word_reg(dev.fd, PMB_BLACK_BOX_PWD, "PMB_BLACK_BOX_PWD", 0);
	}
	if(nvm_pwd){
		pmbus_write_word_reg(dev.fd, PMB_NVM_PWD, "PMB_NVM_PWD", 0xc4c4);
	}else{
		pmbus_write_word_reg(dev.fd, PMB_NVM_PWD, "PMB_NVM_PWD", 0);
	}
